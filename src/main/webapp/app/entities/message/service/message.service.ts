...
type RestOf<T extends IMessage | NewMessage> = Omit<T, 'postDate'> & {
...
export class MessageService {
  ...
  protected convertDateFromClient<T extends IMessage | NewMessage | PartialUpdateMessage>(message: T): RestOf<T> {
    return {
      ...message,
      postDate: message.postDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMessage: RestMessage): IMessage {
    return {
      ...restMessage,
      postDate: restMessage.postDate ? dayjs(restMessage.postDate) : undefined,
    };
  }
